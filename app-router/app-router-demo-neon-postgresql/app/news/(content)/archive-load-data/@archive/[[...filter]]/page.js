    ...
    <header id="archive-header">
      <nav>
        <ul>
          {links.map((link) => {
            const href = year
              ? `/news/archive-load-data/${year}/${link}` //here link is a month
              : `/news/archive-load-data/${link}`; //here link is a year
...
async function FilteredNews({ year, month }) {
  ...
  if (news && news.length > 0) {
    newsContent = <NewsList urlprefix="/news/news-load-data/" news={news} />;
  }
  return newsContent;
}
...
export default async function FilteredNewsPage({ params }) {
  ...
  return (
    <>
      <Suspense
        fallback={selectedMonth ? undefined : <p>loading filters...</p>}
      >
