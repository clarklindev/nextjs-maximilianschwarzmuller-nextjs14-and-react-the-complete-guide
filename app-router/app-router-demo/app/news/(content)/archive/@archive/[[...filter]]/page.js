...
} from "@/lib/news/actions-dummydata";
...
export default function FilteredNewsPage({ params }) {
  ...
  if (news && news.length > 0) {
    newsContent = <NewsList urlprefix="/news/news/" news={news} />;
  }
          ...
          <ul>
            {links.map((link) => {
              const href = selectedYear
                ? `/news/archive/${selectedYear}/${link}` //here link is a month
                : `/news/archive/${link}`; //here link is a year
